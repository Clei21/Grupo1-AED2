...
int combSort(int arr[], int tamanho){
   ...
   int ordenado = 0, trabalhos = 0;

   
   while(!ordenado){
        gap = (int)floor((float)gap / encolhimento);
        if(gap <= 1){  //Caso o intervalo de salto tiver valor 1 ou menor e saiu do loop de iteração o vetor estar ordenado 
            gap = 1; 
            ordenado =1;
        }
        ...
        while((i + gap) <  tamanho){
            ...
            if (arr[i] > arr[iPlusGap]){  
                //Troca de valores, no menor valor de salto haverá uma última iteração para confirmar a ordenação do vetor.
                int temp = arr[i];
                arr[i] = arr[i + gap];
                arr[i + gap] = temp;
                ordenado = 0;                
